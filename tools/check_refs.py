"""Markdown-Link-Validator fuer c-hsm-doc.
...
- Kennungs-Aufloesung (`HSM-*`-Querverweise) und `§`-Sektions-Verweise
...
def _iter_markdown_files(repo_root: Path) -> Iterator[Path]:
    ...
    Andere Top-Level-Verzeichnisse (`scripts/`, `tools/`, `dev/`)
    enthalten heute keine fachlichen Markdown-Querverweise; bei Bedarf
    spaeter erweitern. Root-`*.md` deckt README.md / README.de.md ab.
    """
