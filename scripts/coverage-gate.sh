#!/usr/bin/env bash
# coverage-gate.sh — bootstrap-aware Go coverage gate (ADR 0002 §2.5).
#
# Usage:
#   coverage-gate.sh <coverage-func.txt> <threshold>
#
# Reads the output of `go tool cover -func=<profile>` and enforces the
# overall total coverage against the given threshold (percent, integer
# or float).
#
# Bootstrap mode:
#   When COVERAGE_BOOTSTRAP=1 is set in the environment, an empty input
#   file is treated as "no production code yet" and the gate passes
#   with threshold 0. This avoids a false-green during the bootstrap
#   phase before ./internal/... contains any production packages.
#
#   COVERAGE_BOOTSTRAP=0 (or unset) treats an empty input as a hard
#   failure (exit 2) so that real `go test` regressions cannot be
#   masked.
